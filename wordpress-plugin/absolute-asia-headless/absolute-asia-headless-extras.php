...
const AAT_X_TYPES = ['page', 'post', 'travel-guides', 'places-to-go', 'hotels', 'things-to-do', 'blogs', 'trip', 'cruises', 'travel-ideas'];
...
function aat_x_term_payload($term) {
    ...
    $link = get_term_link($term);
    $acf = function_exists('get_fields') ? (get_fields($term->taxonomy . '_' . $term->term_id) ?: []) : [];
...
function aat_x_get_archive(WP_REST_Request $request) {
    ...
    foreach ($query->posts as $post) {
        ...
        $items[] = [
            ...
            'acf' => function_exists('get_fields') ? (get_fields($post->ID) ?: (object) []) : (object) [],
        ];
    }
