...
function absolute_asia_register_cpts() {
    ...
    $common_taxonomies = array('category', 'post_tag');
    ...
    register_post_type('trip', array(
        'labels' => array('name' => 'Trips (Boats)', 'singular_name' => 'Trip', 'add_new_item' => 'Add New Trip', 'edit_item' => 'Edit Trip'),
        'public' => true, 'has_archive' => true, 'rewrite' => array('slug' => 'trip'),
        'supports' => array('title', 'editor', 'thumbnail', 'excerpt'),
        'taxonomies' => $common_taxonomies,
        'show_in_rest' => true, 'menu_icon' => 'dashicons-ship',
    ));
    register_post_type('cruises', array(
        'labels' => array('name' => 'Cruises', 'singular_name' => 'Cruise', 'add_new_item' => 'Add New Cruise', 'edit_item' => 'Edit Cruise'),
        'public' => true, 'has_archive' => true, 'rewrite' => array('slug' => 'cruises'),
        'supports' => array('title', 'editor', 'thumbnail', 'excerpt'),
        'taxonomies' => $common_taxonomies,
        'show_in_rest' => true, 'menu_icon' => 'dashicons-admin-site-alt3',
    ));
    ...
    // Register Custom Taxonomy: Inspiration
    register_taxonomy('inspiration', array('tour', 'post', 'blog', 'travel_guide'), array(
        'labels' => array(
            'name' => 'Inspirations',
...
function absolute_asia_register_acf_fields() {
    ...
    acf_add_local_field_group(array(
        ...
        'fields' => array(
            ...
            // SingleTourTemplateV2 splits both lists on newlines.
            array('key' => 'field_tour_highlights', 'label' => 'Highlights (One per line)', 'name' => 'highlights_list', 'type' => 'textarea', 'rows' => 6),
    ...
    /* HOMEPAGE CPT FOR ACF FREE COMPATIBILITY */
    register_post_type('homepage', array(
        'labels' => array(
            'name' => 'Homepage',
            'singular_name' => 'Homepage Setting',
            'menu_name' => 'Homepage',
            'add_new' => 'Edit Homepage Data',
            'add_new_item' => 'Add Homepage Data',
        ),
        'public' => false,
        'show_ui' => true,
        'show_in_menu' => true,
    ...
    acf_add_local_field_group(array(
        ...
        'fields' => array(
            ...
            // Hero "Next Up" ticker, read by HomeTemplateV2.
            array('key' => 'field_home_ticker_text', 'label' => 'Hero Ticker Text', 'name' => 'ticker_text', 'type' => 'text'),
            array('key' => 'field_home_ticker_link', 'label' => 'Hero Ticker Link', 'name' => 'ticker_link', 'type' => 'text'),
        ...
        'location' => array(
            array(array('param' => 'post_type', 'operator' => '==', 'value' => 'homepage')),
        ),
        'show_in_rest' => true,
    ));
    ...
    acf_add_local_field_group(array(
        ...
        'fields' => array(
            ...
            /* Directory card lists. The three hub templates each read a differently
               named field and differently named meta column, so they get one
               repeater each rather than a shared one. */
            array('key' => 'field_page_journeys', 'label' => 'Journeys Directory Cards [Repeater]', 'name' => 'journeys', 'type' => 'textarea', 'wrapper' => array('class' => 'custom-free-repeater repeater-type-hub-journeys')),
            array('key' => 'field_page_cruises', 'label' => 'Cruises Directory Cards [Repeater]', 'name' => 'cruises', 'type' => 'textarea', 'wrapper' => array('class' => 'custom-free-repeater repeater-type-hub-cruises')),
            array('key' => 'field_page_articles', 'label' => 'Travel Inspiration Cards [Repeater]', 'name' => 'articles', 'type' => 'textarea', 'wrapper' => array('class' => 'custom-free-repeater repeater-type-hub-articles')),
        ),
        'location' => array(
...
/* ==========================================================================
   ...
   ========================================================================== */
function absolute_asia_repeater_fields() {
    return array(
        ...
        'journeys', 'cruises', 'articles',
    );
}

add_filter('rest_prepare_tour', 'absolute_asia_parse_repeater_json', 10, 3);
add_filter('rest_prepare_page', 'absolute_asia_parse_repeater_json', 10, 3);
add_filter('rest_prepare_homepage', 'absolute_asia_parse_repeater_json', 10, 3);
function absolute_asia_parse_repeater_json($response, $post, $request) {
    if (isset($response->data['acf'])) {
        $fields_to_parse = absolute_asia_repeater_fields();
...
function absolute_asia_format_repeater_for_textarea($value, $post_id, $field) {
    $repeater_fields = absolute_asia_repeater_fields();
...
function absolute_asia_custom_repeater_js() {
    ...
    jQuery(document).ready(function($) {
        ...
        /* Directory hub cards. Each key below is read verbatim by the matching
           Next.js template, so renaming one here breaks that hub's cards. */
        var HUB_SPECS = {
            'hub-journeys': [
        ...
        function initRepeater($wrapper, type) {
            ...
            function renderHubTable() {
                var spec = HUB_SPECS[type];
                var html = '<table class="cfr-table"><thead><tr>';
                spec.forEach(function(col) { html += '<th>' + col.label + '</th>'; });
                html += '<th class="cfr-table-actions"></th></tr></thead><tbody>';
                ...
                data.forEach(function(item, i) {
                    ...
                    spec.forEach(function(col) {
                        ...
                        html += '<td>';
                        if (col.type === 'image') {
                            var bg = raw ? 'background-image: url(' + raw + ');' : '';
                            html += '<div class="cfr-preview upload-img-btn" style="' + bg + '">' + (raw ? '' : 'Click to Upload') + '</div>';
                            html += '<input type="hidden" data-key="' + col.key + '" value="' + esc(raw) + '" />';
                        } else if (col.type === 'textarea') {
                            html += '<textarea data-key="' + col.key + '" rows="5">' + esc(raw) + '</textarea>';
                        } else if (col.type === 'list') {
                            var lines = Array.isArray(raw) ? raw.join('\n') : (raw || '');
                            html += '<textarea data-key="' + col.key + '" data-list="1" rows="4">' + esc(lines) + '</textarea>';
                        } else {
                            html += '<input type="text" data-key="' + col.key + '" value="' + esc(raw) + '" />';
                        }
                        html += '</td>';
                    });
                    html += '<td class="cfr-table-actions"><a href="#" class="cfr-btn-circle cfr-remove-btn" title="Remove Row">-</a></td>';
            ...
            function renderAll() {
                if (HUB_SPECS[type]) {
                    renderHubTable();
                } else if (type === 'home-cards' || type === 'home-banner') {
                    renderTableAll();
                } else {
                    $ui.empty();
                    data.forEach(function(item, i) {
                ...
                $ui.find('.cfr-row').each(function() {
                    ...
                    $(this).find('input, textarea, select').each(function() {
                        ...
                        if ($(this).attr('data-list')) {
                            // The template renders this key with .map(), so it must be an array.
                            obj[k] = String($(this).val()).split('\n').map(function(line) {
                                return line.trim();
        ...
        $('.custom-free-repeater').each(function() {
            ...
            } else if ($this.hasClass('repeater-type-hub-journeys')) {
                initRepeater($this, 'hub-journeys');
            } else if ($this.hasClass('repeater-type-hub-cruises')) {
                initRepeater($this, 'hub-cruises');
            } else if ($this.hasClass('repeater-type-hub-articles')) {
                initRepeater($this, 'hub-articles');
            }
        });
    });
    </script>
